...
# Link: https://github.com/mk-fg/infinite-image-scroller
...
CHANGE_IMAGE_SCALE_FACTOR_BY = 0.03

TEMP_DIR = pl.Path('/tmp/manga_reader')
TEMP_DIR.mkdir(exist_ok=True)

def get_src_paths_iter(_path):
    def getint(name):
        ...
        _int = int(''.join([x if x.isdigit() else '' for x in name]))
        _int = re.findall(r'\d+\.?(?=\d)\d*', name)[0]
        print(_int2)

        if '.jpg' in name and 'images_' in name:
            return int(name.replace('.jpg', '').replace('images_', ''))
        else:
            return 0
    
    src_paths_iter = []
    C_TEMP_DIR = TEMP_DIR / pl.Path(_path).name
    shutil.rmtree(C_TEMP_DIR.as_posix(), ignore_errors=True)
    C_TEMP_DIR.mkdir(exist_ok=True)
    print(C_TEMP_DIR.as_posix(), '|||', _path)
    for dname, dirs, files in os.walk(_path):
        dirs.sort()
        ...
        for fname in files:
            ...
            # print(is_binary_string(open(fpath, 'rb').read(1024)) and (fpath.endswith('.jpg') or fpath.endswith('.png') or fpath.endswith('.jpeg')))
            if is_binary_string(open(fpath, 'rb').read(1024)) and (fpath.endswith('.jpg') or fpath.endswith('.png') or fpath.endswith('.jpeg')):
                # print(fpath)
                src_paths_iter.append(fpath)
            elif fpath.endswith('.cbz') or fpath.endswith('.zip'):
                print(fpath)
                with ZipFile(fpath) as zip:
                    # extracting all the files 
                    # print('Extracting...') 
                    extracted_dir_path = C_TEMP_DIR / pl.Path(fpath).name
                    zip.extractall(extracted_dir_path.as_posix())
                    # print('Done!')
                # src_paths_iter.append(C_TEMP_DIR.as_posix())
                src_paths_iter = get_src_paths_iter(extracted_dir_path.as_posix())
    # print('-'*90)
    return iter(src_paths_iter)
...
class ScrollerWindow(Gtk.ApplicationWindow):
    ...
    def window_key(self, w, ev):
        ...
        if key_name == 'equal': IMAGE_SCALE_FACTOR += CHANGE_IMAGE_SCALE_FACTOR_BY; self.image_set_pixbufs()
        if key_name == 'minus': IMAGE_SCALE_FACTOR -= CHANGE_IMAGE_SCALE_FACTOR_BY; self.image_set_pixbufs()
    ...
    def image_set_pixbufs(self, *ev_args, init=False):
        ...
        sz = getattr(self.get_allocation(), self.dim_scale) * IMAGE_SCALE_FACTOR
...
##				print(sz, self.get_allocation().height)
...
##	else: src_paths_iter = file_iter(src_paths)
    src_paths_iter = get_src_paths_iter(src_paths[0])
